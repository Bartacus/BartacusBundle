...
namespace Bartacus\Bundle\BartacusBundle\Localization;
...
class LocaleSubscriber implements EventSubscriberInterface
{
    ...
    public function onKernelFinishRequest(FinishRequestEvent $event): void
    {
        ...
        // the locale should only be resolved and set for the master request and not for its sub requests too
        if ($parentRequest && $event->isMainRequest()) {
    ...
    private function normalizeLocale(string $locale): string
    {
        return \preg_replace_callback('/^([a-z]{2})[-_]([a-z]{2})$/i', static function (array $matches): string {
            return \sprintf('%s_%s', mb_strtolower($matches[1]), \mb_strtoupper($matches[2]));
        }, $locale);
    }
    ...
    private function getLocaleFromRequest(Request $request): ?string
    {
        ...
        //  - this means this is a TYPO3 page request and we are somewhere inside the TYPO3 middleware stack
